...
class Ego4dDataset(Dataset):
	...
	def __init__(
		...
		if split == 'val':
			self.meta = self.meta[:1000]  # for quick iterations on val set
	...
	def _read_metadata(self, annotations, videos_dir):
		...
		if self.use_processed_data:
			for segment in annotations:
				if self.task != "moments" and "filename" in segment:
					# use full path to video stored under filename key
					video_path = segment["filename"]
				elif self.task == "moments":
					video_path = os.path.join(videos_dir, segment['filename'])
				else:
