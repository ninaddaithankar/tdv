...
class Kinetics400Dataset(Dataset):
    def __init__(self, hparams, split, transform, labels_dir = 'annotations/', dataset_dir=None, ignore_filepath=""):
        """
        Args:
        ...
        self.dataset_dir = dataset_dir if dataset_dir is not None else self.hparams.dataset_dir 
        ...
        self.potential_ignore_filepath = '/'.join([os.getcwd(), "data/cv/corrupt_files/k400.txt"])
        if ignore_filepath:
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            handle_corrupt_file(exception, video_path, log_path, root_dir=self.dataset_dir)
...
class Kinetics600Dataset(Dataset):
    def __init__(self, hparams, split, transform, labels_dir = 'annotations/', dataset_dir=None, ignore_filepath=""):
        ...
        # -- corrupt file list, see data/cv/corrupt_files/<dataset>.txt
        self.potential_ignore_filepath = '/'.join([os.getcwd(), "data/cv/corrupt_files/k600.txt"])
        if ignore_filepath:
            self.ignore_filepath = self.labels_dir + ignore_filepath
        elif os.path.exists(self.potential_ignore_filepath):
            self.ignore_filepath = self.potential_ignore_filepath
