...
dataset_to_root.update(**{
    ...
    'FlyingThings': osp.join(dataset_to_root['SceneFlow'],'FlyingThings3D', 'FlyingThings3D')+'/',
...
def set_dataset_dirs(dataset_dirs):
    ...
    for kv in (dataset_dirs or []):
        ...
        if key == 'SceneFlow':
            dataset_to_root['FlyingThings'] = osp.join(path, 'FlyingThings3D', 'FlyingThings3D') + '/'
...
class FlyingChairsDataset(FlowDataset):
    ...
    def _build_cache(self):
        ...
        assert len(trainpairs)==22232 and len(valpairs)==640, "incorrect parsing of pairs in Flying-Chairs"
