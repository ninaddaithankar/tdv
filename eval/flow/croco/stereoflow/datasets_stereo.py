...
def set_dataset_dirs(dataset_dirs):
    """Override dataset root dirs at runtime.

    Args:
        dataset_dirs: list of 'KEY=PATH' strings, e.g. ['SceneFlow=/data/sf']
    """
    for kv in (dataset_dirs or []):
        key, path = kv.split('=', 1)
        dataset_to_root[key] = path
...
def img_to_tensor(img):
    ...
    img = img.copy()                 # make it writable + contiguous
    img = torch.from_numpy(img).permute(2, 0, 1).float() / 255.0
    img = (img - in1k_mean) / in1k_std
    return img

def unnormalize_imagenet(img_chw):
    """
    img_chw: torch tensor [3,H,W], normalized with ImageNet stats
    returns tensor [3,H,W] in [0,1]
    """
    return (img_chw * in1k_std + in1k_mean).clamp(0, 1)
...
class StereoDataset(data.Dataset):
    ...
    def __init__(self, split, augmentor=False, crop_size=None, totensor=True, data_dir=None):
        self.split = split
    ...
    def _set_root(self, data_dir=None):
...
class SceneFlowDataset(StereoDataset):
    ...
    def _build_cache(self):
        trainpairs = []
        # driving
        pairs = sorted(glob(self.root+'Driving/Driving/frames_finalpass/*/*/*/left/*.png'))
        pairs = list(map(lambda x: x[len(self.root):], pairs))
        assert len(pairs) == 4400, "incorrect parsing of pairs in SceneFlow"
        trainpairs += pairs
        # monkaa
        pairs = sorted(glob(self.root+'Monkaa/Monkaa/frames_finalpass/*/left/*.png'))
        pairs = list(map(lambda x: x[len(self.root):], pairs))
        assert len(pairs) == 8664, "incorrect parsing of pairs in SceneFlow"
        trainpairs += pairs
        # flyingthings
        pairs = sorted(glob(self.root+'FlyingThings3D/FlyingThings3D/frames_finalpass/TRAIN/*/*/left/*.png'))
        pairs = list(map(lambda x: x[len(self.root):], pairs))
        ...
        testpairs = sorted(glob(self.root+'FlyingThings3D/FlyingThings3D/frames_finalpass/TEST/*/*/left/*.png'))
